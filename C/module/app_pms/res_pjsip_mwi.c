...
static int enable_pms = 0;

struct pms_message {
	char                       extension[64];
	int                         message;
	AST_LIST_ENTRY(pms_message) list;
};
static AST_LIST_HEAD_STATIC(pmsmessage, pms_message);
#endif
...
#ifdef GRANDSTREAM_NETWORKS
/* add by kmye for extenal voicemail */
static char* ast_enable_extenal_voicemail(char* mailbox)
{
	char* exten = NULL;
	char* tmp = NULL;
	char* end = NULL;
	...
	if (mailbox == NULL || strlen(mailbox) <= 0) {
		return exten;
	}
	exten = (char*)malloc(64 * sizeof(char));
	if (exten == NULL)
		return exten;
	memset(exten, 0, 64);
	end = strchr(mailbox, '@');
	...
	if (strlen(exten) <= 0)
		return exten;
	ast_log(LOG_DEBUG, "enable_extenal_voicemail exten = %s", exten);
	if (!(endpoint = ast_sorcery_retrieve_by_id(ast_sip_get_sorcery(), "endpoint", exten))) {
		ast_log(LOG_ERROR, "can't find endpoint[%s]", exten);
		free(exten);
		return NULL;
	}
	if (!endpoint || 0 != strncmp(endpoint->remote_mailbox, "yes", 3)) {
		free(exten);
		return NULL;
	}
	return exten;
}
static void update_voicemail_count(const char *exten, struct ast_sip_message_accumulator *counter)
{
	char family[128] = {0};
	char value[64] = {0};
	char tmp_value[64] = {0};
	...
	int  i = 0;
	int len = 0;
	int new_msg = 0, old_msg = 0, urg_old_msg = 0, urg_new_msg = 0;
	if (exten == NULL || counter == NULL)
		return;
	snprintf(family, sizeof(family), "NotifyForwarding/PJSIP");
	ast_db_get(family, exten, tmp_value, sizeof(tmp_value) - 1);
	...
	ast_log(LOG_DEBUG, "value: %s", value);
	if (strlen(value) >= strlen("0/0(0/0)")) {
		sscanf(value, "%d/%d(%d/%d)", &new_msg, &old_msg, &urg_new_msg, &urg_old_msg);
	}
	counter->new_msgs = new_msg;
	counter->old_msgs = old_msg;
	counter->urgent_msgs = (urg_new_msg + urg_old_msg);
	/* send ami to pbxmid. we should update ami_status */
	manager_event(EVENT_FLAG_CALL, "NotifyForwarding",
			"Extension: %s\r\n"
			"Voice-Message: %d/%d (%d/%d)\r\n\r\n",
			exten ? exten : "",
			new_msg, old_msg, urg_new_msg, urg_old_msg
		);
	return;
}
#endif
...
static int get_message_count(void *obj, void *arg, int flags)
{
...
#ifdef GRANDSTREAM_NETWORKS
	/* for notify forwarding by kmye@grandstream.cn*/
	char* exten = NULL;
	if ((exten = ast_enable_extenal_voicemail(mwi_stasis->mailbox)) != NULL) {
		update_voicemail_count(exten, counter);
		free(exten);
	}
	else {
#endif
    ...
    counter->urgent_msgs += mwi_state->urg_msgs;

#ifdef GRANDSTREAM_NETWORKS
	}
	/***add the function of PMS MSG***/
	ast_log(LOG_DEBUG, "handle pms message ...\n");
	if (enable_pms){
		int i = 0;
		char *tmp = NULL;
		char *end = NULL;
		...
		if (mwi_stasis->mailbox){
			end = strchr(mwi_stasis->mailbox, '@');
			if (end != NULL) {
				tmp = mwi_stasis->mailbox;
				while((tmp != end) && (i < sizeof(pms_ext))){
					pms_ext[i] = *tmp;
					tmp++;
					i++;
				}
				ast_debug(1, "the extension of pms room is: %s\n", pms_ext);
			}
			AST_LIST_LOCK(&pmsmessage);
			struct pms_message *cur;
			AST_LIST_TRAVERSE(&pmsmessage, cur, list) {
				if (cur && !strcasecmp(cur->extension, pms_ext)) {
					ast_log(LOG_DEBUG, "the extension of pms room: %s, the message: %d \n", cur->extension, cur->message);
					counter->new_msgs += cur->message;
					break;
				}
			}
			AST_LIST_UNLOCK(&pmsmessage);
		}
	}
#endif
		...
		struct ast_sip_message_accumulator *counter)
{
...
#ifdef GRANDSTREAM_NETWORKS
	if (ast_sip_pubsub_generate_body_content(NULL, body.type, body.subtype, &body_data, &body_text)) {
#else
	...
	ast_debug(5, "Sending unsolicited MWI NOTIFY to endpoint %s, new messages: %d, old messages: %d, urgent messages: %d\n",
			sub->id, counter->new_msgs, counter->old_msgs, counter->urgent_msgs);
...
static int mwi_on_aor(void *obj, void *arg, int flags)
{
	...
	while ((mailbox = strsep(&mailboxes, ","))) {
#ifdef GRANDSTREAM_NETWORKS
		char ext_context[128] = {0};
		snprintf(ext_context,sizeof(ext_context),"%s@default", mailbox);
		RAII_VAR(struct mwi_stasis_subscription *, mwi_stasis_sub,
				mwi_stasis_subscription_alloc(ext_context, sub), ao2_cleanup);
#else
		RAII_VAR(struct mwi_stasis_subscription *, mwi_stasis_sub,
...
#ifdef GRANDSTREAM_NETWORKS
static void load_pms_destroy(void)
{
	AST_LIST_LOCK(&pmsmessage);
	struct pms_message *cur;
	AST_LIST_TRAVERSE_SAFE_BEGIN(&pmsmessage, cur, list) {
...
static void load_pms_config(struct ast_config *cfg, const char *message)
{
	AST_LIST_LOCK(&pmsmessage);
	const char *tmpvar;
	if (message && (tmpvar = ast_variable_retrieve(cfg, message, "message"))) {
		struct pms_message *tmp = ast_calloc(1, sizeof(*tmp));
		if (tmp) {
			ast_copy_string(tmp->extension, message, sizeof(tmp->extension));
			tmp->message = atoi(tmpvar);
			ast_log(LOG_DEBUG, "tmp->extension: %s, tmp->message: %d\n", tmp->extension, tmp->message);
			AST_LIST_INSERT_TAIL(&pmsmessage, tmp, list);
		}
	}
	AST_LIST_UNLOCK(&pmsmessage);
}
...
static int load_config(void)
{
	...
	if (!(cfg = ast_config_load("res_pjsip_mwi.conf", config_flags))) {
		ast_log(LOG_NOTICE, "No res_pjsip_mwi config file (res_pjsip_mwi.conf)\n");
		return -1;
	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		return 0;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log(LOG_ERROR, "Config file res_pjsip_mwi.conf is in an invalid format.  Aborting.\n");
		return -1;
	}

	cat = NULL;
	while ((cat = ast_category_browse(cfg, cat))) {
		ast_log(LOG_DEBUG, "cat: %s\n", cat);
		if (!strcasecmp(cat, "general")) {
			const char *general_val = NULL;
			if ((general_val = ast_variable_retrieve(cfg, "general", "enable_pms"))) {
				enable_pms = atoi(general_val);
				ast_log(LOG_DEBUG, "enable_pms: %d\n", enable_pms);
			}
			continue;
		}

		load_pms_config(cfg, cat);
	}
