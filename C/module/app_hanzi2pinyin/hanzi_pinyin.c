...
const char * path_filename = "./hanzi.txt";
...
int main(int argc, char *argv[])
{
    ...
    unsigned char read_buff[HZ2PY_FILE_READ_BUF_ARRAY_SIZE];
    unsigned char inbuf[HZ2PY_INPUT_BUF_ARRAY_SIZE];
    unsigned char outbuf[HZ2PY_OUTPUT_BUF_ARRAY_SIZE];
    ...
    while ((opt = getopt(argc, argv, "BPfdh")) != -1)
    {
        switch(opt)
        {
            case 'B':
            ...
            case 'h':
printf("hz2py %s\n\
Read utf8 string from stdin and convert chinese charactors to pinyin.\n\
\n\
...
eg: ./hz2py -d\n\
\n\
Author: chzhdeng@grandstream.com\n", HZ2PY_VERSION );
                return 0;
                break;
            default:
                break;
        ...
        memset(read_buff, '\0', sizeof(char) * HZ2PY_FILE_READ_BUF_ARRAY_SIZE);
        ...
        handle = open(path_filename,O_RDONLY) ;
		if ( handle == -1 )
		{
		printf("[%s] create fail !!!!",path_filename) ;
		return 1 ;
		}

        number = read(handle, read_buff, HZ2PY_FILE_READ_BUF_ARRAY_SIZE - 1) ;
